    ...
    resources.use {
      case (blockchainUpdater, _) =>
        for {
          ...
          utxPool      = new UtxPoolImpl(ntpTime, blockchainUpdater, ignoreSpendableBalanceChanged, ignoreSpendableBalanceChanged, utxSettings, getBadAssetsDiff = (_, _) => Map.empty)
