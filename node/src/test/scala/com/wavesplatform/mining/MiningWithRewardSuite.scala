    ...
    resources(settings).use {
      case (blockchainUpdater, _) =>
        for {
          ...
          utxPool      = new UtxPoolImpl(ntpTime, blockchainUpdater, ignoreSpendableBalanceChanged, ignoreSpendableBalanceChanged, settings.utxSettings, getBadAssetsDiff = (_, _) => Map.empty, enablePriorityPool = true)
